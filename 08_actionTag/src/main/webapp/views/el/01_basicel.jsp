...
<body>
	<h2>EL표현식 사용하기</h2>
	<p>EL은 <%-- ${} --%>를 사용해서 <%-- ${} --%>내부에 있는 리터럴값,
	내장객체에 저장된 값,vo에 저장된 값을 출력하는 용도로 사용</p>
	<h4>리터럴값 출력하기</h4>
	<p><%="리터럴값 출력" %></p>
	<p>${"리터럴값 출력"}</p>
	<p>${19}</p>
<%-- 	<p>${new Date().toString()}</p> --%>
	...
	<p>
		EL표현식으로 특정한 값을 호출하면 자동으로
		내장객체(request,session,application,pageContext)에 저장된 데이터를 찾음
	</p>
	<% String name="유병승"; 
		request.setAttribute("name", name);
	%>
	<p>${name }</p><!--내장객체에 저장을 해줘야 이렇게 쓸 수 있음 -->
	...
	<%
		...
		Student s1=new Student("이동훈",26,190.5,'남');
		Student s2=new Student("송이현",26,165.5,'여');
	...
	<p>복합 : ${su+10-su2/su3*2 }</p>
	
	<h4>비교연산활용하기</h4>
	<h5>대소비교하기</h5>
	<p>&lt; : ${su < su2 } ${su lt su2 }</p>
	<p>&gt; : ${su > su2 } ${su gt su2 }</p>
	<p>&le; : ${su2 <= su3 } ${su2 le su3 }</p>
	<p>&ge; : ${su2 >= su3 } ${su2 ge su3 }</p>
	...
	<p>${testData2} : ${testData2 == null } ${testData2 eq null }</p>
	
	<p>List값 확인하기 : ${students == null } ${students.size() > 0 } 
					${!students.isEmpty() }</p>
	<p>empty예약어 사용하기 : ${empty students } ${not empty students }</p>
	
	<h5>삼항연산자 사용</h5>
	<p>삼항연산자 : ${su>19?"성인입니다":"미성년입니다" }</p>
	<p>삼항연산자 : ${testData eq "admin"?"관리자입니다.":"일반회원입니다." }</p>
	<input type="checkbox" name="adminCheck" 
			${testData eq "admin"?"checked":"" }>관리자
	...
	<p>${su<10 and testData eq "admin" }</p>
	<p>${su<10 or testData eq "admin" }</p>
	<p>${su<10 || testData eq "admin" }</p>
	
		
<%-- 	
	<c:if test="${testData eq "admin" }">
	</c:if> 
--%>
	
	<h4>기타 제공되는 데이터 출력하기</h4>
	<p>header의 정보,  cookie의 정보, context정보를 가져올 수 있음</p>
	<%-- <%=request.getContextPath() %> --%>
	<a href="${pageContext.request.contextPath}/el/extraData.do">추가 데이터 출력</a>
