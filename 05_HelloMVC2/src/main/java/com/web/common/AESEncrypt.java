...
public class AESEncrypt {
	...
	public AESEncrypt() {
		...
		//2. 생성된 키가 없으면(key클래스를 저장한 파일이 없으면) 키를 생성 후 파일로 저장
	...
	private void getGeneratedKey() {
		...
		//KeyGenerator클래스를 생성 -> key 생성해주는 역할
		KeyGenerator keygen=null;
		try(ObjectOutputStream oos
				=new ObjectOutputStream(new FileOutputStream(this.path+"bslove.bs"))){
	...
	public static String encryptData(String oriVal) throws Exception {
		...
		byte[] targetData=oriVal.getBytes();
